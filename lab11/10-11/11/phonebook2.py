...
def create_table(conn):
    try:
        ...
        print("Error creating PhoneBook2 table:", e)

def insert_data(conn, first_name, last_name, phone_number):
    try:
        cur = conn.cursor()
        cur.execute("""INSERT INTO Phonebook2 (first_name, last_name, phone_number)
...
def update_data(conn, user_id, first_name=None, last_name=None, phone_number=None):
    try:
        cur = conn.cursor()
        update_query = "UPDATE Phonebook2 SET "
        update_values = []
...
def delete_data_by_firstname(conn, first_name):
    try:
        cur = conn.cursor()
        delete_query = "DELETE FROM Phonebook2 WHERE first_name = %s;"
...
def insert_or_update_user(conn, first_name, phone_number):
    try:
        cur = conn.cursor()
        cur.execute("""SELECT id FROM Phonebook2 WHERE first_name = %s""", (first_name,))
...
def get_users_with_pagination(conn, limit_rows, offset_rows):
    try:
        cur = conn.cursor()
        cur.execute("""SELECT * FROM Phonebook2 LIMIT %s OFFSET %s""", (limit_rows, offset_rows))
        rows = cur.fetchall()
        if rows:
            print("Query results:")
            for row in rows:
...
def delete_data_by_username_or_phone(conn, value):
    try:
        cur = conn.cursor()
        cur.execute("""DELETE FROM Phonebook2 WHERE first_name = %s OR phone_number = %s""", (value, value))
        conn.commit()
        print("Data deleted successfully")
    except psycopg2.Error as e:
        print("Error deleting data:", e)
